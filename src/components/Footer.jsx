...
const Footer = () => {
  // destrusture footer data
  const { contact, hours, social } = footerData;

  return (
    <footer id="footer" className="relative top-96 z-20 bg-dark lg:bg-transparent lg:min-h-[620px] lg:bg-footer lg:bg-center lg:bg-no-repeat lg:bg-cover pt-20">
                ...
                {hours.program.map((item, index) => {
                  return (
                    <div className="capitalize" key={index}>
                      <div>
                        <div>{item.days}</div>
                        <div className="text-[20px] text-accent/60">
                          {item.hours}
                        </div>
                      </div>
                    </div>
